...
export class PokerHand extends Hand {
    ...
    hasStraight() {
        const rankValues = this.cards.map(c => this.rankValue(c.rank)).sort((a, b) => a - b);
    ...
    classify() {
        if (this.hasStraightFlush())
            this.classification = 'Straight Flush';
        else if (this.hasFourOfAKind())
            this.classification = 'Four of a Kind';
        else if (this.hasFullHouse())
            this.classification = 'Full House';
        else if (this.hasFlush())
            this.classification = 'Flush';
        else if (this.hasStraight())
            this.classification = 'Straight';
        else if (this.hasThreeOfAKind())
            this.classification = 'Three of a Kind';
        else if (this.hasTwoPair())
            this.classification = 'Two Pair';
        else if (this.hasPair())
            this.classification = 'Pair';
        else
            this.classification = 'High Card';
    ...
    compareWith(other) {
        ...
        for (let i = 0; i < thisSorted.length; i++) {
            if (thisSorted[i].isHigherThan(otherSorted[i]))
                return 1;
            if (otherSorted[i].isHigherThan(thisSorted[i]))
                return -1;
        }
        return 0;
    }
